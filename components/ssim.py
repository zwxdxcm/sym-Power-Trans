...
def _ssim(img1, img2, window, window_size, channel, size_average = True, stride=3):
...
def ssim(img1, img2, window_size = 11, size_average = True, stride=3):
    (_, channel, _, _) = img1.size()
...
def compute_ssim_loss(img1, img2):
    # align shape
    img1 = img1.permute(2,0,1).unsqueeze(0).to(torch.float32) # 1,c,h,w
    img2 = img2.permute(2,0,1).unsqueeze(0).to(torch.float32)
